...
class model_cov_4PCF(object):
    ...
    def __init__(self, meta_data=None, fname_f3l=None, fname_GLs=None, shot_noise=None,
                 k_in=None, Pk_in=None,
                 vol=None, r_in=None, ndim=None, verbose=False):
        ...
        if k_in is None:
            ...
            self.k_in = k_in
        if Pk_in is None:
            self.Pk_in = meta_data.pk0_in
        else:
            self.Pk_in = Pk_in
        self.verbose = verbose
        
        if vol is None:
            self.vol = meta_data.vol
        else:
            self.vol = vol
        if shot_noise is None:
            try:
                self.shot_noise = meta_data.shot_noise
                print("use Shot-noise from meta_data")
            except:
                print("Shot-noise is zero")
                self.shot_noise = 0
        else:
            self.shot_noise = shot_noise
            print("use input shot_noise")
        power = 2
        print("damping factor: exp(-k**%.f)"%power)
        self.Pk_in = (self.Pk_in  + self.shot_noise) * numpy.exp(-self.k_in**power)
#        self.Pk_in = (self.Pk_in + self.shot_noise) * numpy.exp(-1.*(self.k_in/.5)**4.)
        ...
        if ndim is not None:
            ...
            self.ndim = len(self.meta_data.rbins)
    ...
    def permute_like_prof(self, r_in, idx_six):
    ...
    def permute_ell_GH(self, lls, lls_p, which_group, idx_six):
        ...
        l_Gs = self.permute_like_prof(lls, idx_six)
    ...
    def wrap_rs_fc(self, r_in, which_group, idx_six):
        ...
        r1, r2, r3 = self.permute_like_prof([r1, r2, r3], idx_six)
    ...
    def io_GLs(self, action='load', fname=None, verbose=False):
        ...
        if action == 'save':
            ...
            keys = []
            GLs_dict = {}
            for ii, iell_g in enumerate(self.ells_comb2):
    ...
    def get_4pcf_cov_model(self, idx_min=0, idx_max=1, groups=None, save_name=None, fname_ext=None):
        ...
        for ii, iell_g in enumerate(self.ells_comb2[idx_min:idx_max]):
            l1, l2, l3 = [int(i) for i in iell_g[0]]
            l1_p, l2_p, l3_p = [int(i) for i in iell_g[1]]
            lls = [l1, l2, l3]
            lls_p = [l1_p, l2_p, l3_p]
            ...
            ic = 0
            self.y_arr_24 = numpy.zeros([self.ndim,self.ndim,24])
            self.yint_arr = numpy.zeros([self.ndim, self.ndim])
                        ...
                        for ir1_p in self.rr[:-2]:
                            for ir2_p in self.rr[self.rr>ir1_p][:-1]:
                                for ir3_p in self.rr[self.rr>ir2_p]:
                                    rbin_i, rbin_j = ic//self.ndim, ic%self.ndim
                                    if ic%2000==0: print(int(ic//2000)*"*"+"%.f"%(ic/self.ndim**2*100)+"%","ic=",ic, end="\r")
                                    ic += 1
            ...
            ## Store meta_data that used to
            ## compute this covariance
            self.cov_dict['attrs'] = {}
            self.cov_dict['attrs']['rbins'] = self.rr
            self.cov_dict['attrs']['shot_noise'] = self.shot_noise
            self.cov_dict['attrs']['volume'] = self.vol
            self.cov_dict['attrs']['fname_f3l'] = self.fname_f3l
            self.cov_dict['attrs']['fname_gls'] = self.fname_GLs

            if save_name is not None:
                fdir = "/Users/mianoctobers/Projects/NPCF/cov/"
                if fname_ext is None:
#                    fname = fdir + "4pcf_real_z2_full_couple_%sx%s.cov"%(key[0],key[1])
                    fname = fdir + save_name + "_%sx%s.cov"%(key[0],key[1])
                else:
                    fname = fdir + save_name + "_" + fname_ext + "_%sx%s.cov"%(key[0],key[1])
                f = open(fname, "wb")
                pickle.dump(self.cov_dict, f)
                f.close()
...
def estimate_time(cov_model_4pcf_zs):
    ll_pps_list = []
    ic = 0
    for ii, iell_g in enumerate(cov_model_4pcf_zs.ells_comb2):
        l1, l2, l3 = [int(i) for i in iell_g[0]]
        l1_p, l2_p, l3_p = [int(i) for i in iell_g[1]]
        lls = [l1, l2, l3]
        lls_p = [l1_p, l2_p, l3_p]
    ...
    print(ic)
    #     print(self.ells_comb2[ii], len(ll))
